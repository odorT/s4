...
def databases():
    if request.method == 'POST':
        ...
        provisioner.destroy()
        row_todelete = Databases.query.filter_by(instance_id=id).first()
        # deleted = Databases.query.filter_by(instance_id=id).delete()
        print(">>>>>>>>heree<<<<", row_todelete)
        db.session.delete(row_todelete)
        db.session.commit()
    ...
    if 'instance_id' in request.form:
        ...
        if instance:
            return render_template('home/create-database.html',
                msg=f"Instance with id {instance_id} already exists, please choose another id",
                success=False,
                form=database_form
            )

        sql_instance = Provision(instance_id=instance_id)
        result = sql_instance.create(password=request.form['password'], capacity=request.form['capacity'], database_type=request.form['database'])
